...
namespace Asteroids.Tools
{
    ...
    public abstract class Generator : Entity
    {
        ...
        protected ScrollBehavior scrollMode; // What does this generator's objects do on scroll?
        protected CollisionBehavior collideMode; // What does this generator's objects do on collision?
        ...
        public Generator(ShapeDrawer _pen, Texture2D _image, EntityType _type) : base(_pen, "Generator", _image, _type)
        {
            Initialize(_type, _bounds: null);
        }
        public Generator(ShapeDrawer _pen, Texture2D _image, EntityType _type, Rectangle? _bounds = null, Vector2? _dir = null, bool _scrollable = true,
            int _max = MAX_ENTITIES, int _iteration = MIN_ITERATION) : base(_pen, "Generator", _image, _type)
        {
            Initialize(_type, _bounds, _dir, _scrollable);
        }

        public Generator(ShapeDrawer _pen, Texture2D _image, EntityType _type, Vector2? _pos = null, Vector2? _dir = null, Vector2? _size = null, bool _scrollable = true) : base(_pen, "Generator", _image, _type)
        {
            Initialize(_type, _pos, _dir, _size, _scrollable);
        }

        public Generator(ShapeDrawer _pen, Texture2D _image, EntityType _type, Vector2? _pos = null, Vector2? _dir = null, Vector2? _size = null, bool _scrollable = true,
           int _max = MAX_ENTITIES, int _iteration = MIN_ITERATION) : base(_pen, "Generator", _image, _type)
        {
            Initialize(_type, _pos, _dir, _size, _scrollable, _max, _iteration);
        }
        ...
        public Vector2 GetDirection()
        {
            if (IsEmpty(this.direction))
            {
                // Generate new values.
                Random r = InputManager.RNG;
                float x = r.Next(1, (int)Game1.ScreenBounds.X + 1);
                float y = r.Next(1, (int)Game1.ScreenBounds.Y + 1);
        ...
        // A generator should take care of its entities update calls.
        public abstract override void UpdateGUI(GameTime gameTime);
        ...
        {
            return ((!Empty()) && (objects.Count() == max));
        }
        ...
        protected virtual void Initialize(EntityType _type, Rectangle? _bounds = null, Vector2? _dir = null, bool _scrollable = true, int _max = MAX_ENTITIES, int _iteration = MIN_ITERATION)
        {
            this.visible = false;
        ...
        protected virtual void Initialize(EntityType _type, Vector2? _pos = null, Vector2? _dir = null, Vector2? _dim = null, bool _scrollable = true, int _max = MAX_ENTITIES, int _iteration = MIN_ITERATION)
        {
            this.visible = false;
            this.collisionsOn = false;
            this.scrollable = _scrollable;
       ...
       /* protected virtual void SetType(EntityType _type)
        ...
        {
            this.max = MathHelper.Clamp(_max, MIN_ENTITIES, MAX_ENTITIES);
        }

        // Generate this many artifacts at one time.
        protected virtual void SetIteration(int _spawn)
        {
            this.iteration = MathHelper.Clamp(_spawn, MIN_ITERATION, max);
        }
        ...
        protected virtual void SetPosition(Vector2? _pos = null)
        {
            if (IsEmpty(_pos))
            {
                this.position = Game1.ScreenCenter;
            }
            else
            {
        ...
        {
            this.direction = Vector2.Normalize(Game1.ScreenBounds - Game1.ScreenBounds);
